package br.com.vinicius.mestrado.metaheuristica.utils.cec15;
...
public class testfunc {
    ...
    public void test_func(int func_num, int dim) throws Exception {
        int cf_num, i, j;
        ...
        if (!(dim == 2 || dim == 10 || dim == 20 || dim == 50 || dim == 100)) {
        ...
        /*Load Matrix M*****************************************************/
        File fpt = new File("input_data/M_" + func_num + "_D" + dim + ".txt");//* Load M data *
        ...
        if (func_num < 9) {
            ...
            for (i = 0; i < cf_nums[func_num] - 1; i++) {
                for (j = 0; j < dim; j++) {
                    OShift[i * dim + j] = Double.parseDouble(input.next());
                }
                input.nextLine();

            }
            for (j = 0; j < dim; j++) {
                OShift[(cf_nums[func_num] - 1) * dim + j] = Double.parseDouble(input.next());
            }
        ...
        if (bShuffle[func_num] == 1) {
            fpt = new File("input_data/shuffle_data_" + func_num + "_D" + dim + ".txt");
            input = new Scanner(fpt);
            if (!fpt.exists()) {
                System.out.println("\n Error: Cannot open input file for reading ");
            }
    ...
    public double callFunction(double[] coords, int dim, int func_num, int shift, int rotate) {
        double result = 0;
        switch (func_num) {
            case 1:
                result = ellips_func(coords, result, dim, OShift, M, shift, rotate);
                result += 100.0;
                break;
            case 2:
                result = bent_cigar_func(coords, result, dim, OShift, M, shift, rotate);
                result += 200.0;
                break;
            case 3:
                result = ackley_func(coords, result, dim, OShift, M, shift, rotate);
                result += 300.0;
                break;
            case 4:
                result = rastrigin_func(coords, result, dim, OShift, M, shift, rotate);
                result += 400.0;
                break;
            case 5:
                result = schwefel_func(coords, result, dim, OShift, M, shift, rotate);
                result += 500.0;
                break;
            case 6:
                result = hf01(coords, result, dim, OShift, M, SS, shift, rotate);
                result += 600.0;
                break;
            case 7:
                result = hf02(coords, result, dim, OShift, M, SS, shift, rotate);
                result += 700.0;
                break;
            case 8:
                result = hf03(coords, result, dim, OShift, M, SS, shift, rotate);
                result += 800.0;
                break;
            case 9:
                result = cf01(coords, result, dim, OShift, M, bias, rotate);
                result += 900.0;
                break;
            case 10:
                result = cf02(coords, result, dim, OShift, M, SS, bias, rotate);
                result += 1000.0;
                break;
            case 11:
                result = cf03(coords, result, dim, OShift, M, bias, rotate);
                result += 1100.0;
                break;
            case 12:
                result = cf04(coords, result, dim, OShift, M, bias, rotate);
                result += 1200.0;
                break;
            case 13:
                result = cf05(coords, result, dim, OShift, M, SS, bias, rotate);
                result += 1300.0;
                break;
            case 14:
                result = cf06(coords, result, dim, OShift, M, bias, rotate);
                result += 1400.0;
                break;
            case 15:
                result = cf07(coords, result, dim, OShift, M, bias, rotate);
                result += 1500.0;
                break;
    ...
    double happycat_func(double[] x, double f, int nx, double[] Os, double[] Mr, int s_flag, int r_flag) /*HappyCat, probided by Hans-Georg Beyer (HGB)*/ /*original global optimum: [-1,-1,...,-1]*/ {
        int i;
    ...
    double hgbat_func(double[] x, double f, int nx, double[] Os, double[] Mr, int s_flag, int r_flag) /*HGBat, provided by Hans-Georg Beyer (HGB)*/ /*original global optimum: [-1,-1,...-1]*/ {
        int i;
        double alpha, r2, sum_z;
        alpha = 1.0 / 4.0;
